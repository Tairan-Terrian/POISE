...
def safe_wandb_log(*args, **kwargs):
    ...
    try:
        ...
        if e.errno == errno.ENOSPC:
            ...
            print("⚠️ 磁盘空间不足，跳过Wandb日志记录")

        else:

            print(f"❌ OS错误，跳过Wandb日志记录: {e}")

        pass # 跳过

    except Exception as e:

        # 捕获所有其他可能的异常 (例如 wandb API 错误)

        print(f"❌ 其他错误，跳过Wandb日志记录: {e}")
...
def save_ckpt(save_dir, transformer, global_step, accelerator, ema, transformer_trainable_parameters, config):
    ...
    try:
        ...
        # (os.makedirs 在 is_local_main_process 中执行更安全)
        ...
        if accelerator.is_main_process:
            ...
            print(f"✅ 成功保存检查点到: {save_root_lora}")
        ...
        if e.errno == errno.ENOSPC:
            ...
            print(f"⚠️ 磁盘空间不足，跳过检查点保存: {save_root_lora}")

        else:

            print(f"❌ OS错误，跳过检查点保存: {e}")

        pass # 跳过

    except Exception as e:

        print(f"❌ 其他错误，跳过检查点保存: {e}")
...
def main(_):
    ...
    while True:
        ...
        record_samples = {
            "rewards": {
                k: v.clone() for k, v in samples["rewards"].items()  # imagereward, clip, etc.
            }
        }

        # 替换 avg 部分为 flowcpo 处理后的
        record_samples["rewards"]["ori_avg"] = samples["rewards"]["avg"].clone()
        record_samples["rewards"]["avg"] = samples["rewards"]["avg"].unsqueeze(1).repeat(1, num_train_timesteps)

        # gather并转到cpu
        record_rewards = {key: accelerator.gather(value) for key, value in record_samples["rewards"].items()}
        record_rewards = {key: value.cpu().numpy() for key, value in record_rewards.items()}

        if accelerator.is_main_process:
            safe_wandb_log(
                {
                    "epoch": epoch,
                    **{f"reward_{key}": value.mean() for key, value in record_rewards.items()
                       if '_strict_accuracy' not in key and '_accuracy' not in key},
                },
                step=global_step,
            )
        ############################################
        # >>> Flow-CPO LOGIC + reward processing
        ############################################

        avg_rewards = samples["rewards"]["avg"]  # (batch_size,)
        group_size_total = config.sample.num_image_per_prompt
        subgroup_size = 4
        # subgroup_size = 4

        num_samples = avg_rewards.shape[0]
        num_prompts = num_samples // group_size_total

        reshaped = avg_rewards.view(num_prompts, group_size_total)
        flowcpo_rewards = torch.zeros_like(avg_rewards)

        for i in range(num_prompts):
            prompt_rewards = reshaped[i]
            num_subgroups = group_size_total // subgroup_size
            for j in range(num_subgroups):
                start = j * subgroup_size
                end = start + subgroup_size
                subgroup = prompt_rewards[start:end]

                ranks = torch.argsort(torch.argsort(subgroup, dim=0), dim=0)
                normalized_rank = ranks.float()  # ← 不归一化，直接用rank

                flowcpo_rewards[i * group_size_total + start: i * group_size_total + end] = normalized_rank

        samples["rewards"]["ori_avg"] = avg_rewards.clone()
        samples["rewards"]["avg"] = flowcpo_rewards
        samples["rewards"]["avg"] = samples["rewards"]["avg"].unsqueeze(1).repeat(1, num_train_timesteps)
        ...
        # ############################################ 
        # # 原始 reward (每张图片一个值) 
        # avg_rewards = samples["rewards"]["avg"] # shape: (batch_size,) 
        # # ===== Flow-CPO rank-based reward ===== 
        # group_size = config.sample.num_image_per_prompt # 每个 prompt 的图片数量，通常为4 
        # num_samples = avg_rewards.shape[0]
        # num_groups = num_samples // group_size # (num_groups, group_size)
        # reshaped = avg_rewards.view(num_groups, group_size) 
        # # 对每组内reward从高到低排名（越高reward rank越大） 
        # ranks = torch.argsort(torch.argsort(-reshaped, dim=1), dim=1) 
        # # 将rank归一化到[0,1]区间，形成FlowCPO替换reward 
        # normalized_rank = ranks.float() / (group_size - 1) 
        # # 展平成(batch_size,) 
        # flowcpo_rewards = normalized_rank.view(-1) 
        # # 记录原始reward 
        # samples["rewards"]["ori_avg"] = avg_rewards.clone() 
        # # 替换reward为rank-based版本 
        # samples["rewards"]["avg"] = flowcpo_rewards 
        # # ========== 
        # samples["rewards"]["avg"] = samples["rewards"]["avg"].unsqueeze(1).repeat(1, num_train_timesteps)
        ...
        if accelerator.is_main_process:
            safe_wandb_log(
                {
                    "var_advantages": samples["advantages"].var(),
                },
                step=global_step,
            )
            ...
            for i, sample in tqdm(
                ...
                for j in tqdm(
                    ...
                    if accelerator.sync_gradients:
                        ...
                        if accelerator.is_main_process:
                            safe_wandb_log(info, step=global_step)
                        global_step += 1
                        info = defaultdict(list)
                if config.train.ema:
