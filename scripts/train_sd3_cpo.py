...
def eval(pipeline, test_dataloader, text_encoders, tokenizers, config, accelerator, global_step, reward_fn, executor, autocast, num_train_timesteps, ema, transformer_trainable_parameters):
    ...
    for test_batch in tqdm(
        ...
        rewards = executor.submit(reward_fn, images, prompts, prompt_metadata, config, only_strict=False)
...
def save_ckpt(save_dir, transformer, global_step, accelerator, ema, transformer_trainable_parameters, config):
    ...
    if accelerator.is_main_process:
        ...
        if config.train.ema:
            ema.copy_temp_to(transformer_trainable_parameters)
# ===================== FLOW-CPO 核心：Qwen-VL-Chat AI Judge =====================

# === 加载模型 ===       
from deepseek_vl.models import VLChatProcessor, MultiModalityCausalLM
model_path = "./deepseek-vl-7b-base"
_processor = None
_model = None

def _load_deepseek_vl_once():
    global _processor, _model
    if _processor is None or _model is None:
        # print("▶ 正在加载 deepseek 模型（首次加载，约 1~3 分钟）...")
        _processor = VLChatProcessor.from_pretrained(model_path)
        _model = MultiModalityCausalLM.from_pretrained(
            model_path,
...
def ensure_pil(image):
    if isinstance(image, Image.Image):
        ...
        # 处理 4D (B,C,H,W) 或 3D (C,H,W) 或 2D (H,W)
        if image.ndim == 4:
...
# ==== FLOW-CPO MODIFICATIONS ==== 
# ============= 1. 定义 AI 评委函数 (AI Judge) =============
# === AI Judge Function ===
def ai_judge_group(images, prompt, group_size=4, max_retries=3):
    """
    Use deepseek_vl to rank 4 images based on the given prompt.
    """
    _load_deepseek_vl_once()

    # 确保 images 全是 PIL.Image.Image 对象
    ...
    conversation = [
        {
            "role": "User",
            "content": f"Please rank these four images based on their relevance to the following description: {prompt}. "
                       "Output must be a JSON list of four numbers, e.g., [0, 1, 2, 3], where 0 is the best and 3 is the worst. "
    ...
    for attempt in range(max_retries):
        try:
            # Use correct conversation format with the processor
            inputs = _processor(
                conversations=conversation,
                images=images,  # list of 4 PIL images or tensors
                force_batchify=True
            ).to('cuda' if torch.cuda.is_available() else 'cpu')

            inputs_embeds = _model.prepare_inputs_embeds(**inputs)
            ...
            with torch.no_grad():
                outputs = _model.language_model.generate(
                    ...
                    bos_token_id=_processor.tokenizer.bos_token_id,
            ...
            output_text = _processor.tokenizer.decode(outputs[0].cpu().tolist(), skip_special_tokens=True)

            # Extract the first valid JSON array
            match = re.search(r'(\[\s*\d+\s*,\s*\d+\s*,\s*\d+\s*,\s*\d+\s*\])', output_text)
            if not match:
                raise ValueError("No valid JSON array found in response")

            rankings = json.loads(match.group(1))
            # print(f"Raw rankings from model output: {rankings}")
            # 容错处理：如果不是全排列，则重新映射为0~3的顺序
            if sorted(rankings) != [0, 1, 2, 3]:
                # 保留原顺序，但映射到 0,1,2,3
                sorted_unique = sorted(set(rankings))
                mapping = {v: i for i, v in enumerate(sorted_unique)}
                rankings = [mapping[v] for v in rankings]

            if not isinstance(rankings, list) or len(rankings) != 4:
                raise ValueError("Output must be a list of 4 integers")

            if sorted(rankings) != [0, 1, 2, 3]:
                raise ValueError("Ranks must be a permutation of [0,1,2,3]")
            ...
            if attempt == max_retries - 1:
                raise RuntimeError(f"Failed to get valid ranking from deepseek_vl after {max_retries} attempts: {e}")
            time.sleep(2 ** attempt)  # Exponential backoff

    raise RuntimeError("Unreachable")

# ============= 2. 修改 reward_fn 为 CPO 排名函数 =============
# 替换 flow_grpo.rewards.multi_score 为我们的分数/排名生成器
def cpo_reward_fn(images, prompts, prompt_metadatas, config, only_strict=False):
    """
    CPO 的核心：我们不返回分数，而是返回每个图像的“组内排名”。
    我们将 G=24 拆成 6 组，每组4张，每组由 ai_judge_group 排序。
    输出格式：{ 'avg': [24个排名值] }，训练时依据排名计算优势
    """
    # 确保 G 是 config.sample.num_image_per_prompt
    G = len(images)  # 按 batch size * G
    # print(f"Debug: G = {G}, Prompt: num = {len(prompts)} Expected: {config.sample.num_image_per_prompt}")
    assert G % 4 == 0, "G must be divisible by 4 for grouping"
    num_groups = G // 4

    # 创建输出列表：存储每一个图像在所属小组中的排名
    all_ranks = []

    # 按 batch 分组处理（一个 prompt 对应 G 个图像）
    for i in range(len(prompts)):
        ...
        for g in range(num_groups):
            group_imgs = batch_images[g*4: (g+1)*4]
            rank_in_group = ai_judge_group(group_imgs, prompt, group_size=4)  # [4] 长度列表
            group_ranks.extend(rank_in_group)  # 合并到全局排名

        # 每个图像对应一个排名值，保存为 int（0=最好）
        assert len(group_ranks) == G, "Group ranks length mismatch"
        all_ranks.extend(group_ranks)

    # 返回为 { 'avg': rankings } —— 和原框架的 'avg' 同结构，只是现在是排名而非得分
    return {'avg': np.array(all_ranks)}, { 'avg': 'CPO ranking (lower=better)' }
...
def main(_):
    ...
    # 替换reward_fn,用定义好的cpo_reward_fn 
    reward_fn = cpo_reward_fn
    ...
    while True:
        ...
        for i in tqdm(
            ...
            rewards = executor.submit(reward_fn, images, prompts, prompt_metadata, config, only_strict=True)
            # yield to to make sure reward computation starts
            time.sleep(0)
        ...
        if epoch % 10 == 0 and accelerator.is_main_process:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                # 用 CPO 的排名来显示，不是分数，但为了显示，我们将其转为（排名越小，值越大）的可视化分数
                avg_rank = samples["rewards"]["avg"].cpu().numpy()

                wandb.log({
                    "images": [
                        wandb.Image(
                            os.path.join(tmpdir, f"{idx}.jpg"),
                            caption=f"{prompt:.100} | CPO_RANK: {avg_rank[i]:.1f}"
                        )
                        for idx, (prompt, avg_rank_i) in enumerate(zip(sampled_prompts, avg_rank))
                    ]
                }, step=global_step)
        ...
        # ==== FLOW-CPO MODIFICATIONS ==== 
        # ★★★★ 核心修改：使用排名计算优势，不是分数
        # 我们现在有 samples["rewards"]["avg"] 是一个长度为 G*B 的排名列表（0=最好，3=最差）
        gathered_rewards = {key: accelerator.gather(value) for key, value in samples["rewards"].items()}
        gathered_rewards = {key: value.cpu().numpy() for key, value in gathered_rewards.items()}

        # 2. 成为新的优势计算：基于排名
        # 注意：排名是【0,1,2,3】，越小越好，所以：为了符合传统优势公式，我们计算：
        # advantage = (mean_rank - current_rank) / std_rank
        # 这样：好图=正优势，差图=负优势
        current_rank = gathered_rewards['avg']
        mean_rank = np.mean(current_rank)
        std_rank = np.std(current_rank) + 1e-4
        advantages = (mean_rank - current_rank) / std_rank  #
        ...
        for k, v in samples.items():
            if v.shape[0] == 1:
                ...
                # batch>1，确保 mask 和 batch 维匹配
                if mask.numel() == 1:
                    mask_use = mask.expand(v.shape[0])
                else:
                    # 保证 mask 长度不大于 batch
                    mask_use = mask[:v.shape[0]]

                # 如果 mask 是 integer 索引，确保不会越界
                if mask_use.dtype == torch.int64 or mask_use.dtype == torch.int32:
                    mask_use = mask_use.clamp(0, v.shape[0] - 1)

                samples[k] = v[mask_use]
