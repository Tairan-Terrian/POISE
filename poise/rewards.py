...
def geneval_score(device):
    ...
    import importlib
    gv_mod = importlib.import_module("geneval.geneval")
    # prefer load_geneval() factory
    if hasattr(gv_mod, "load_geneval"):
        compute_fn = gv_mod.load_geneval()
    elif hasattr(gv_mod, "compute_geneval"):
        compute_fn = gv_mod.compute_geneval
    else:
        raise ImportError("no load_geneval / compute_geneval found in flow_grpo.geneval.geneval")

    def _fn(images, prompts, metadatas, only_strict):
        # normalize images -> list[PIL.Image]
        pil_images = []
        if isinstance(images, torch.Tensor):
            ...
            # assume already iterable of PIL.Image
            pil_images = list(images)

        # compute_fn expected args: (image_pils, metadatas, only_strict)
        return compute_fn(pil_images, metadatas, only_strict)
